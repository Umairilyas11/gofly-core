  ...
  class CSF_Field_typography extends CSF_Fields {
      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Family', 'gofly-core' ) .'</div>';
            echo $this->create_select( array( $this->value['font-family'] => $this->value['font-family'] ), 'font-family', esc_html__( 'Select a font', 'gofly-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Backup Font Family', 'gofly-core' ) .'</div>';
            echo $this->create_select( apply_filters( 'csf_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'gofly-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Style', 'gofly-core') .'</div>';
            echo '<select class="csf--font-style-select" data-placeholder="Default">';
            echo '<option value="">'. ( ! $this->chosen ? esc_html__( 'Default', 'gofly-core' ) : '' ) .'</option>';
            ...
            if ( ! empty( $args['extra_styles'] ) ) {
              ...
              echo ( ! $this->chosen ) ? '<div class="csf--title">'. esc_html__( 'Load Extra Styles', 'gofly-core' ) .'</div>' : '';
              $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'gofly-core' ) : esc_html__( 'Default', 'gofly-core' );
          ...
          if ( ! empty( $args['subset'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Subset', 'gofly-core' ) .'</div>';
            $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
            echo $this->create_select( $subset, 'subset', esc_html__( 'Default', 'gofly-core' ), $args['multi_subset'] );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Align', 'gofly-core' ) .'</div>';
            echo $this->create_select( array(
              'inherit' => esc_html__( 'Inherit', 'gofly-core' ),
              'left'    => esc_html__( 'Left', 'gofly-core' ),
              'center'  => esc_html__( 'Center', 'gofly-core' ),
              'right'   => esc_html__( 'Right', 'gofly-core' ),
              'justify' => esc_html__( 'Justify', 'gofly-core' ),
              'initial' => esc_html__( 'Initial', 'gofly-core' )
            ), 'text-align', esc_html__( 'Default', 'gofly-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Variant', 'gofly-core' ) .'</div>';
            echo $this->create_select( array(
              'normal'         => esc_html__( 'Normal', 'gofly-core' ),
              'small-caps'     => esc_html__( 'Small Caps', 'gofly-core' ),
              'all-small-caps' => esc_html__( 'All Small Caps', 'gofly-core' )
            ), 'font-variant', esc_html__( 'Default', 'gofly-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Transform', 'gofly-core' ) .'</div>';
            echo $this->create_select( array(
              'none'       => esc_html__( 'None', 'gofly-core' ),
              'capitalize' => esc_html__( 'Capitalize', 'gofly-core' ),
              'uppercase'  => esc_html__( 'Uppercase', 'gofly-core' ),
              'lowercase'  => esc_html__( 'Lowercase', 'gofly-core' )
            ), 'text-transform', esc_html__( 'Default', 'gofly-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Decoration', 'gofly-core' ) .'</div>';
            echo $this->create_select( array(
              'none'               => esc_html__( 'None', 'gofly-core' ),
              'underline'          => esc_html__( 'Solid', 'gofly-core' ),
              'underline double'   => esc_html__( 'Double', 'gofly-core' ),
              'underline dotted'   => esc_html__( 'Dotted', 'gofly-core' ),
              'underline dashed'   => esc_html__( 'Dashed', 'gofly-core' ),
              'underline wavy'     => esc_html__( 'Wavy', 'gofly-core' ),
              'underline overline' => esc_html__( 'Overline', 'gofly-core' ),
              'line-through'       => esc_html__( 'Line-through', 'gofly-core' )
            ), 'text-decoration', esc_html__( 'Default', 'gofly-core' ) );
            echo '</div>';
          }
        ...
        echo '<div class="csf--blocks csf--blocks-inputs">';
          ...
          if ( ! empty( $args['font_size'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Size', 'gofly-core' ) .'</div>';
          ...
          if ( ! empty( $args['line_height'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Line Height', 'gofly-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['letter_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Letter Spacing', 'gofly-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['word_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Word Spacing', 'gofly-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
        ...
        if ( ! empty( $args['color'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Font Color', 'gofly-core' ) .'</div>';
        ...
        if ( ! empty( $args['custom_style'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Custom Style', 'gofly-core' ) .'</div>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        wp_enqueue_script( 'csf-webfontloader', 'https://cdn.jsdelivr.net/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'gofly-core' ), '1.6.28', true );
        ...
        if ( ! empty( $customwebfonts ) ) {
          $webfonts['custom'] = array(
            'label' => esc_html__( 'Custom Web Fonts', 'gofly-core' ),
            'fonts' => $customwebfonts
          );
        }

        $webfonts['safe'] = array(
          'label' => esc_html__( 'Safe Web Fonts', 'gofly-core' ),
        ...
        $webfonts['google'] = array(
          'label' => esc_html__( 'Google Web Fonts', 'gofly-core' ),
        ...
        wp_localize_script( 'gofly-core', 'csf_typography_json', array(
          'webfonts'      => $webfonts,
